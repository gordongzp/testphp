...
class SellerCenterController extends Controller {
	...
	public function openShop(){
		...
		if ($config['openshop_need_person_id']) {
			if (session('user.person_identity_stage')!=3) {
				$this->error('需要进行实名认证','/Home/UserCenter/identityId',2);
			}
		}
		//判断是否需要开店认证
		if ($config['openshop_need_verify']) {
			if (session('user.shop_identity_stage')!=3) {
		...
		if (IS_POST) {
			...
			$this->success('开店成功','/Home/UserCenter/index',2);
		} else {
			$this->display();
		}
	}
	...
	public function shopVerify(){
		...
		if (IS_POST) {
	    	...
	    	//set状态identity_stage以及身份证+truename；
		   	...
		   	$this->assign('shop_identity_stage',session('user.shop_identity_stage'));
		   	$this->display();	
		   }
		}
	}